{ service-name, service-pkg, tiddlywiki-pkg, config, lib, pkgs }:
let
...
in
  let
    ...
    service_config = import ./config { inherit service-name service-description tiddlywiki-pkg cfg lib pkgs; };
  in
    {
      options = {
        services = {
